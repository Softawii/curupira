...
public class Curupira extends ListenerAdapter {
        ...
        for(Class group : groups) {
            ...
            if(hidden) {
                ...
                for(Class innerClass : innerClasses) {
                    ...
                    String name = isubgroup.name();
                    String desc = isubgroup.description();

                    LOGGER.info(String.format("Registering CommandGroup (Sub)", name));

                    CommandDataImpl commandData = new CommandDataImpl(name, desc);
            ...
            // Not Hidden -> Creating Group Command
                ...
                String name = igroup.name().toLowerCase();
                String desc = igroup.description();
                CommandDataImpl commandData = new CommandDataImpl(name, desc);
    ...
    private CommandDataImpl getCommandFromMethod(Method method) {
        ...
        // TODO: Link to mappers

        // if(type == Type.SLASH)
        // TODO: Add Else (if slash or not)
    ...
    private SubcommandData getSubcommandFromMethod(Method method) {
        ...
        if(type != Type.SLASH) throw new RuntimeException("Subcommand must be of type SLASH");
